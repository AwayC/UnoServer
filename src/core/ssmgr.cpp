...
namespace uno
{
    ...
    void Ssmgr::accept(WsSessionPtr& socket)
    {
        ...
        socket->onMessage([this, weak_ss](WsSessionPtr socket)
        {
            if (auto ss = weak_ss.lock())
            {
                ...
                try
                {
                    std::string evt = args.get_element(0).get_string();
                    if (evt == "c2s")
                    {
                        on_c2s_msg(ss, args);
                    }
                ...
                {
                    std::cerr << "invalid msg: " << e.what() << std::endl;
                }
            }
    ...
    void Ssmgr::on_c2s_msg(SessionPtr& session, const lept_value& args)
    {
        ...
        try
        {
            funcname = args.get_element(2).get_string();
            ...
            {
                const std::string& token = args.get_element(2).get_string();
            ...
            {
                const std::string& title = args.get_element(2).get_string();
                int play_count = args.get_element(3).get_integer();
                C2S::create_room_req(session, title, play_count);
            }
            else if (funcname == "enter_room_req")
            {
                int room_id = args.get_element(2).get_integer();
                bool re_enter = args.get_element(3).get_boolean();
            ...
            {
                int room_id = args.get_element(2).get_integer();
                int voice_id = args.get_element(3).get_integer();
                C2S::room_use_voice_req(session, room_id, voice_id);
            }
            else if (funcname == "room_kick_player_req")
            {
                int room_id = args.get_element(2).get_integer();
                bool be_kicked = args.get_element(3).get_boolean();
            ...
            {
                int room_id = args.get_element(2).get_integer();
                double c = args.get_element(3).get_number();
                bool with_uno = args.get_element(4).get_boolean();
                int chg_color = args.get_element(5).get_integer();
